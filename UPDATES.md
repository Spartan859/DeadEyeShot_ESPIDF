...
## 2026-07-30: 蜂鸣器改为有源驱动 (分支: codex/magazine-reload)

### 问题
原实现按无源蜂鸣器设计，使用 LEDC 输出不同频率的方波。更换为有源蜂鸣器后，蜂鸣器内部自带振荡源，不需要也不应继续通过改变 PWM 频率播放音符。

### 新方法: GPIO 电平节拍

1. GPIO14 配置为普通推挽输出，高电平打开蜂鸣器，低电平关闭蜂鸣器。
2. 提示音由开、关状态及持续时间组成，不再包含频率参数。
3. 开机提示为三次鸣响，前两次短鸣，最后一次稍长。
4. Wi-Fi 连接成功提示为一次短鸣加一次长鸣，与开机提示区分。
5. 保留原有 FreeRTOS 队列和异步播放任务，调用方与主流程无需修改。
6. 每个节拍播放结束后强制输出静音电平，避免蜂鸣器持续鸣响。

### 变更文件
- `main/buzzer.c` - 移除 LEDC 频率驱动，改为 GPIO 高低电平节拍
